...
export default function AssetForm({ asset, type, onClose, onSaved }) {
  ...
  useEffect(() => { getFieldDefs(type.id).then(setDefs).catch(e => setErr(e.message)) }, [type.id])
  ...
  return (
    <Modal size="lg" title={isNew ? `Nuevo · ${type.name}` : `Editar · ${asset.name}`} onClose={onClose}
      ...
      {defs.length === 0 && <p className="hint">Este tipo no tiene campos definidos. Usa el botón <b>Campos</b> para agregarlos.</p>}
    </Modal>
  )
}
